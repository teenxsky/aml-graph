"""Автоматический выбор алгоритмов обработки графа.

Все решения (метод кластеризации, способ вычисления центральностей,
веса риск-скоринга) принимаются автоматически на основе характеристик
графа. Пользователь не выбирает алгоритмы — он получает результат.

Логика выбора и обоснования экспортируются в метаданные анализа для
прозрачности: investigator может посмотреть, какой именно метод
применялся и почему.
"""
